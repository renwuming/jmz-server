...
router.get('/wx/:id',
  ...
  const bodyData = {
    ...
    lanjieUser: game.userList[lanjieIndex]
      ? game.userList[lanjieIndex].userInfo
      : {},
    actionPaperIndex,
...
function getGameHistory(gameData) {
...
router.post('/:id/submit', sessionUser, getGame, async (ctx, next) => {
  ...
  if (type) {
    ...
    if (activeBattle <= 1 && battle.question && battle.answerF) {
      // 若为前两轮，只需要队友解密
...
router.post('/wx/:id/submit', sessionUser, getGame, async (ctx, next) => {
  ...
  if (type) {
    ...
    if (activeBattle <= 1 && newBattleData.question && newBattleData.answerF) {
      // 若为前两轮，只需要队友解密
      ...
      newBattleData.question &&
      newBattleData.answerF &&
      newBattleData.answerE
    ) {
      const codeF = newBattleData.answerF.join('')
